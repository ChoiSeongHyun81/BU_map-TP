...
export default function PlaceDetail({
  ...
  const [activeTab, setActiveTab] = useState<"info" | "review" | "photo">("info");
    ...
    <div className="mt-4 bg-white rounded-2xl shadow-inner w-full max-w-md overflow-hidden">
      <div className="flex border-b border-gray-200">
        {["info", "review", "photo"].map((tab) => (
          <button
            key={tab}
            onClick={() => setActiveTab(tab as "info" | "review" | "photo")}
      ...
      <div className="p-4 text-sm text-gray-700 space-y-2">
        ...
        {activeTab === "review" && <div className="text-gray-500 italic">추가 필요</div>}
        {activeTab === "photo" && <div className="text-gray-500 italic">추가 필요</div>}
      </div>
    </div>
  );
}